// Fonts for guests/dat.html (Baloo 2 + Be Vietnam Pro). Downloads Google Fonts (latin + vietnamese subsets) and writes fonts.css
// with base64 data-URI @font-face rules, so the invite page is self-contained.
const fs = require('fs');
const path = require('path');

const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0 Safari/537.36';
const CSS_URL = 'https://fonts.googleapis.com/css2?family=Baloo+2:wght@700&family=Be+Vietnam+Pro:wght@400;500;700&display=swap';
...
async function main() {
  ...
  fs.writeFileSync(path.join(__dirname, '..', 'scratch', 'dat-fonts.css'), out);
